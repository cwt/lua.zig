...
pub fn findupval(L: *lua.lua_State, idx: usize) !*lua.UpVal {
    const target_ptr = &L.stack[idx];
    const target_addr = @intFromPtr(target_ptr);
    var prev: ?*lua.UpVal = null;
    ...
    const uv = try L.allocator.create(lua.UpVal);
    uv.* = .{
        .value = .{ .nil = {} },
        .v = target_ptr,
        .next = curr,
...
pub fn closeCallFailed(L: *lua.lua_State, e: anyerror, tm: lua.TValue, old_top: usize) anyerror!?lua.TValue {
    ...
    const saved_err = if (L.top > old_top and L.top - 1 < L.stack.len) L.stack[L.top - 1] else lua.TValue{ .nil = {} };
    L.top = old_top;
    if (e == error.NotAFunction) {
        ...
        const msg = lua.fmtMsg(&buf, "attempt to call a bad value (metamethod 'close')", "attempt to call a {s} value (metamethod 'close')", .{tname});
        const ts = lstring.luaS_new(L, msg) catch {
            return saved_err;
        };
        return lua.TValue{ .string = ts };
