...
| B7 | hub cleanup ✅ DONE (2026-09-10) | `src/lua.zig` left with: **core type definitions only** (`TValue`, `lua_Table`, closures, `UpVal`, `CallInfo`, `VMGCObject`, `global_State` *definition*, `lua_State` *definition*) + constants + the thin re-export tail (`pub const precall = ldo.precall; …`). The `global_State`/`lua_State` *constructors* live in B5. The 7 residual hub functions relocated to their C-file homes (`l_alloc`/`G` -> lstate.zig, `createProto`/`destroyProto` -> lcode.zig, `findupval`/`closeCallFailed` -> ldo.zig, `fmtMsg` -> lobject.zig), each re-exported. `lua.zig` ends at 886 lines: **0 function definitions** - core types + constants + re-export tail. The monolith that was 6253 lines at the start of Phase A is now a pure hub. | ~800 |
